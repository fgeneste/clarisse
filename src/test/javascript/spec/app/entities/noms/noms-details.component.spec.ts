/* tslint:disable max-line-length */
import { shallowMount, createLocalVue, Wrapper } from '@vue/test-utils';
import sinon, { SinonStubbedInstance } from 'sinon';
import VueRouter from 'vue-router';

import * as config from '@/shared/config/config';
import NomsDetailComponent from '@/entities/noms/noms-details.vue';
import NomsClass from '@/entities/noms/noms-details.component';
import NomsService from '@/entities/noms/noms.service';
import router from '@/router';
import AlertService from '@/shared/alert/alert.service';

const localVue = createLocalVue();
localVue.use(VueRouter);

config.initVueApp(localVue);
const i18n = config.initI18N(localVue);
const store = config.initVueXStore(localVue);
localVue.component('font-awesome-icon', {});
localVue.component('router-link', {});

describe('Component Tests', () => {
  describe('Noms Management Detail Component', () => {
    let wrapper: Wrapper<NomsClass>;
    let comp: NomsClass;
    let nomsServiceStub: SinonStubbedInstance<NomsService>;

    beforeEach(() => {
      nomsServiceStub = sinon.createStubInstance<NomsService>(NomsService);

      wrapper = shallowMount<NomsClass>(NomsDetailComponent, {
        store,
        i18n,
        localVue,
        router,
        provide: { nomsService: () => nomsServiceStub, alertService: () => new AlertService() },
      });
      comp = wrapper.vm;
    });

    describe('OnInit', () => {
      it('Should call load all on init', async () => {
        // GIVEN
        const foundNoms = { id: 123 };
        nomsServiceStub.find.resolves(foundNoms);

        // WHEN
        comp.retrieveNoms(123);
        await comp.$nextTick();

        // THEN
        expect(comp.noms).toBe(foundNoms);
      });
    });

    describe('Before route enter', () => {
      it('Should retrieve data', async () => {
        // GIVEN
        const foundNoms = { id: 123 };
        nomsServiceStub.find.resolves(foundNoms);

        // WHEN
        comp.beforeRouteEnter({ params: { nomsId: 123 } }, null, cb => cb(comp));
        await comp.$nextTick();

        // THEN
        expect(comp.noms).toBe(foundNoms);
      });
    });

    describe('Previous state', () => {
      it('Should go previous state', async () => {
        comp.previousState();
        await comp.$nextTick();

        expect(comp.$router.currentRoute.fullPath).toContain('/');
      });
    });
  });
});
