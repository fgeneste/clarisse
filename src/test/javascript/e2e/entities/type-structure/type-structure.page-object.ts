...
export default class TypeStructureComponentsPage extends AlertPage {
  title: ElementFinder = element(by.id('type-structure-heading'));
...
export class TypeStructureDeleteDialog {
  dialog: ElementFinder = element(by.id('removeEntity'));
  title: ElementFinder = element(by.id('clarisseApp.typeStructure.delete.question'));
  confirmButton: ElementFinder = element(by.id('jhi-confirm-delete-typeStructure'));
}
