...
export default class AppartenanceComponentsPage extends AlertPage {
  title: ElementFinder = element(by.id('appartenance-heading'));
...
export class AppartenanceDeleteDialog {
  dialog: ElementFinder = element(by.id('removeEntity'));
  title: ElementFinder = element(by.id('clarisseApp.appartenance.delete.question'));
  confirmButton: ElementFinder = element(by.id('jhi-confirm-delete-appartenance'));
}
